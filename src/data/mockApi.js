/**
 * Premium Mock API layer simulating asynchronous HTTP requests for KoinX Assignment
 */

// Capital Gains baseline mock database
...
// Holdings list mock database (exact dataset provided in assignment specs)
const MOCK_HOLDINGS = [
  {
    coin: "USDC",
    coinName: "USDC",
...
/**
 * Simulates fetching Capital Gains baseline data with custom network delay.
 * 
 * @param {number} delayMs - Optional custom network delay
 * @returns {Promise<object>} Baseline Capital Gains object
 */
export const getCapitalGainsData = (delayMs = 600) => {
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      // Simulate highly stable resolver
...
/**
 * Simulates fetching Holdings list data with custom network delay.
 * 
 * @param {number} delayMs - Optional custom network delay
 * @returns {Promise<Array>} Holdings array list
 */
export const getHoldingsData = (delayMs = 800) => {
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      // Return a deep cloned array copy to prevent unexpected mutations
      const holdingsCopy = MOCK_HOLDINGS.map((item, index) => ({
        ...item,
        // Assign a solid unique identifier
        id: `${item.coin}-${index}`,
        // Ensure nested objects are copied
        stcg: { ...item.stcg },
        ltcg: { ...item.ltcg }
      }));
      resolve(holdingsCopy);
    }, delayMs);
  });
};
